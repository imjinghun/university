...
<body>
<form name="form1" action="shypw.jsp" method="post" onsubmit="return check()">
<p>
...
<jsp:useBean id="db" class="bean.DBBean" scope="page" />
	...
	<%
		...
		if (submit != null && !submit.equals("")) {
			...
			if (i == 1) {
				out.println("<script language='javaScript'> alert('修改成功,点击确定转到审核员界面!');</script>");
				response.setHeader("refresh", "1;url=shy.jsp");
			} else {
				out.println("<script language='javaScript'> alert('修改失败，点击确定返回当前页面！');</script>");
				response.setHeader("refresh", "1;url=shypw.jsp");
			}
			}
			db.close();
		}
