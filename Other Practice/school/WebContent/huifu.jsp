...
<form name="form1" action="useradd.jsp" method="post" onsubmit="return check()">
	...
	<%
		...
		if (submit != null && !submit.equals("")) {
			...
			if (i == 1) {
				out.println("<script language='javaScript'> alert('回复留言成功,点击确定转到用户界面!');</script>");
				response.setHeader("refresh", "1;url=user.jsp");
			} else {
				out.println("<script language='javaScript'> alert('回复留言失败，点击确定返回回复页面！');</script>");
				response.setHeader("refresh", "1;url=huifu.jsp");
			}
			db.close();
		}
	%>
