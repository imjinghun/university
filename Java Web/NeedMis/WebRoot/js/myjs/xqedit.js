...
function gjzSet(){
	var gjz=document.getElementById("needkey").value;
	var g=gjz.split(" ");
...
function load2()
{
	...
	//根据通信地址选择所在地域
	var dz=document.getElementById("contactaddr").value.toString();
	if(dz!="")
	{
		...
		if(dy!="")
		{
			var select=document.getElementById("locationArea");
	...
	if(gkglbm!="")
	{
		var gkbm=gkglbm.substring(3);
		var select=document.getElementById("parentmgt");
    	var options=select.options;
    		
    	for(var i=0;i<options.length;i++)
    	{
    		if(options[i].text==gkbm)
    		{
    			select.options[i].selected = true; 
    			break;
    		}
...
function check(){
	...
	if(form1.locationArea.value=="select")
	{
		form1.locationArea.focus();
    ...
    //机构属性
    var first=document.getElementsByName("organattr");
	var jgsx=""; 
    ...
    if (form1.organInfo.value.trim() == "") {
        form1.organInfo.focus();
        layer.msg('机构简介不能为空',{time:1200});
     
        return false;
    }
    if (form1.needname.value.trim() == "") {
    ...
    if(!xqsx())
    {
    	form1.needsyear.focus();
    	layer.msg('开始年份格式不正确',{time:1200});
        return false;
    } 
    if(!xqsx2())
    {
    	form1.needeyear.focus();
    	layer.msg('结束格式不正确',{time:1200});
        return false;
    }
    if (form1.needoverview1.value.trim() == "") {
        form1.needoverview1.focus();
        layer.msg('技术需求概述 主要问题 不能为空',{time:1200});
        
        return false;
    }
    if (form1.needoverview2.value.trim() == "") {
        form1.needoverview2.focus();
        layer.msg('技术需求概述 技术关键 不能为空',{time:1200});
       
        return false;
    }
    if (form1.needoverview3.value.trim() == "") {
        form1.needoverview3.focus();
    ...
    document.getElementById("needkey").value=gjz;
    ...
    //技术需求解决方式
    var third=document.getElementsByName("needmodel");
	var jjfs=""; 
    ...
    //科技活动类型
    var fourth=document.getElementsByName("restype");
	var hdlx=""; 
	...
	if(fourth[0].checked == true)
	{
		...
		//设置技术应用行业 为空
		document.getElementById("needindustry").value="";
		document.getElementById("needfield").value="";
    ...
    if(fourth[0].checked == false)
    {
    	...
    	document.getElementById("needfield").value=xqly;
...
function xqsx()
{
	var date=new Date();
	re=/^\d{4}$/;
	result=re.test(form1.needsyear.value);
	if(!result)
	{
		form1.needsyear.focus();
		layer.msg('开始年份格式不正确',{time:1200});
	...
	if(Number(form1.needsyear.value)<Number(date.getFullYear()))
	{
		form1.needsyear.focus();
		layer.msg('开始年份格式不正确 ',{time:1200});
...
{
	var re=/^\d{4}$/;
	result=re.test(form1.needeyear.value);
	if(!result)
	{
		form1.needeyear.focus();
	...
	if(Number(form1.needeyear.value)<=Number(form1.needsyear.value))
	{
		layer.msg('结束年份格式不正确 ',{time:1200});
		form1.needeyear.focus();
		return false;
...
function emailcheck()
{
	...
	var result=re.test(form1.email.value);
	if(!result)
	{
		layer.msg('电子信箱格式错误 ',{time:1200});
		form1.email.focus();
...
function ybcheck()
{
	...
	{
		result=re.test(form1.postcode.value);
		if(!result)
		{
...
function ghcheck()
{
	...
	if(form1.tel.value!="")
	{
		result=re.test(form1.tel.value);
		if(!result)
		{
...
function sjcheck()
{
	//手机号 11位
	re=/^\d{11}$/;
	result=re.test(form1.phone.value);
	if(!result)
	{
		layer.msg('手机格式错误 ',{time:1200});
		form1.phone.focus();
...
function zjzecheck()
{
	if(form1.totalmoney.value != "")
	{
		//资金 为整数或小数
		re=/^[0-9]+([.]{1}[0-9]+){0,1}$/;
    	result=re.test(form1.totalmoney.value);
    	if(!result)
    	{
...
function selLimit()
{
	var yl=document.getElementsByName("restype");
	//选择 基础研究
...
function selLimit2()
{
	var hs=document.getElementsByName("needmodel");
	if(hs[0].checked==true)
	{
		document.getElementById("coopunit").style.display="none";
	}
	else
	{
		document.getElementById("coopunit").style.display="block";
	}
}