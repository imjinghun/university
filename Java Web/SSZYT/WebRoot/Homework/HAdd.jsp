		...
		if(inputcheck()){
			$.ajax({
				...
				success:function(retData){
					if(retData=="成功")
					{
						layer.msg("作业发布成功，1秒后自动关闭",{time:1000});
						
						setTimeout(function(){parent.location.reload();},1000);
					}
					else{
						layer.msg("作业发布异常，1秒后重试",{time:1000});
					}
				}
			});
		}
		...
		<div class="divcenter">
			<table>
				<tr>
					...
					<td><input placeholder="最多50字" type="text" class="input"
						maxlength="50" name="hname" id="hname" onkeyup="textnum()" />
						还可输入<label id="num">50</label>字</td>
				</tr>
				<tr>
					<th scope="row">作业内容</th>
					<td><textarea name="hinfo" id="hinfo" rows="25"
							class="textarea1"></textarea></td>
				</tr>
				<tr>
					...
					<td>
						<div class="demo2">
							<input placeholder="选择日期" class="laydate-icon" id="hendtime"
								name="hendtime" onClick="laydate(start)" readonly>
						</div>
					</td>
				</tr>
				<tr>
					<td colspan="2" align="center">
						<button onclick="add('<%=hcid%>')" class="btn">确认</button></td>
				</tr>
			</table>
